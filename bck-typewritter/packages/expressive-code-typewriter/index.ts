...
const BASE_STYLES = `
  ...
  /* Clip-path is now set dynamically via JavaScript to handle wrapped lines */
  /* Fallback for non-wrapped single lines */
  .ec-typed .ec-line:not([data-wrapped]) .code {
    clip-path: inset(0 calc(100% - var(--ec-typed-chars) * 1ch) 0 0);
  }

  /* For wrapped lines, clip-path is set via inline style in JS */
  .ec-typed .ec-line[data-wrapped] .code {
    /* clip-path set dynamically */
  }
  ...
  class TypedBlock {
    ...
    cacheLineData() {
      ...
      this.lineElements.forEach((lineEl, index) => {
        ...
        // Get the code element dimensions to detect wrapping
        const codeElRect = codeEl.getBoundingClientRect();

        // Detect if line is wrapped by checking if height > single line height
        const isWrapped = codeElRect.height > lineHeight * 1.5;

        // Calculate characters per visual line for wrapped content
        let charsPerVisualLine = 0;
        if (isWrapped && this.linesData[index]) {
          const totalChars = this.linesData[index].charCount;
          const numVisualLines = Math.round(codeElRect.height / lineHeight);
          charsPerVisualLine = Math.ceil(totalChars / numVisualLines);

          // Mark the line element as wrapped for CSS targeting
          lineEl.setAttribute('data-wrapped', 'true');
        }
        ...
        this._lineCache[index] = {
          ...
          isWrapped: isWrapped,
          charsPerVisualLine: charsPerVisualLine
        };
      });
    }
    ...
    generateWrappedClipPath(cache, visibleChars, totalChars) {
      // Calculate dimensions
      const lineHeight = cache.lineHeightPx;
      const charWidth = cache.charWidth;
      const charsPerLine = cache.charsPerVisualLine || Math.floor(cache.codeElRect.width / charWidth);

      if (charsPerLine <= 0) {
        return 'none';
      }

      // Calculate which visual line we're on and position within it
      const currentVisualLine = Math.floor(visibleChars / charsPerLine);
      const charsOnCurrentLine = visibleChars % charsPerLine;

      // Calculate total visual lines
      const totalVisualLines = Math.ceil(totalChars / charsPerLine);

      // Build polygon points for an "L" or staircase shape
      // The shape reveals complete lines and partial current line
      const points = [];
      ...
      if (currentVisualLine === 0) {
        // Still on first visual line - simple rectangle
        const rightEdge = (charsOnCurrentLine || visibleChars) * charWidth;
        points.push(rightEdge + 'px 0');
        points.push(rightEdge + 'px 100%');
        points.push('0 100%');
      } else {
        // Multiple visual lines - create staircase shape
        // Top edge - full width for completed lines
        points.push('100% 0');

        // Right edge down to current line
        const completedLinesBottom = currentVisualLine * lineHeight;
        points.push('100% ' + completedLinesBottom + 'px');

        // Step in to current line position
        const currentLineRight = (charsOnCurrentLine === 0 ? charsPerLine : charsOnCurrentLine) * charWidth;
        points.push(currentLineRight + 'px ' + completedLinesBottom + 'px');

        // Down to bottom of current line (or hide remaining)
        const currentLineBottom = (currentVisualLine + 1) * lineHeight;
        points.push(currentLineRight + 'px ' + currentLineBottom + 'px');

        // Back to left edge
        points.push('0 ' + currentLineBottom + 'px');
      }
      ...
      for (const timing of this.timings) {
        ...
        if (elapsed < timing.startTime) {
          ...
          if (timing.isInput) {
            ...
            // Update clip-path based on whether line is wrapped
            const cache = this._lineCache[timing.index];
            if (cache && cache.isWrapped) {
              // Use polygon clip-path for wrapped lines
              const clipPath = this.generateWrappedClipPath(cache, visibleChars, timing.charCount);
              const codeEl = cache.codeEl;
              if (codeEl) {
                codeEl.style.clipPath = clipPath;
              }
            } else {
              // Use CSS variable for non-wrapped lines
    ...
    resetAnimation() {
      ...
      this.lineElements.forEach((el, index) => {
        ...
        // Reset clip-path for wrapped lines
        const codeEl = el.querySelector('.code');
        if (codeEl) {
          codeEl.style.clipPath = '';
        }
      });
